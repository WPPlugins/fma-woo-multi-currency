...
if ( !class_exists( 'FMA_Multi_Currency_Front' ) ) {
	...
	class FMA_Multi_Currency_Front extends FMA_Multi_Currency {
		...
		function __construct() {
			...
			add_filter( 'woocommerce_available_variation', array($this, 'filter_woocommerce_variation_price_html'), 100, 3 );
			...
			if($query_chk_list!='') { ?>
				...
				<?php
					if($product->product_type == 'variable') {
						...
						$min_vari_price = get_post_meta($product->id, '_min_variation_price', true);
						$max_vari_price = get_post_meta($product->id, '_max_variation_price', true);

						$min_vari_reg_price = get_post_meta($product->id, '_min_variation_regular_price', true);
						$max_vari_reg_price = get_post_meta($product->id, '_max_variation_regular_price', true);

						$min_vari_sale_price = get_post_meta($product->id, '_min_variation_sale_price', true);
						$max_vari_sale_price = get_post_meta($product->id, '_max_variation_sale_price', true);
					...
					<?php
						if($min_vari_sale_price!='') {

							if($min_vari_reg_price == $max_vari_reg_price) { ?>

							<del>
								<span class="woocommerce-Price-currencySymbol"><?php echo $query_chk_list['currency_symbol']; ?></span>
								<?php echo number_format($max_vari_reg_price * $query_chk_list['currency_value'], $decimails); ?>
							</del>

							<ins>
								<span class="woocommerce-Price-amount amount">
								<span class="woocommerce-Price-currencySymbol"><?php echo $query_chk_list['currency_symbol']; ?></span>
								<?php echo number_format($min_vari_sale_price * $query_chk_list['currency_value'], $decimails); ?>
								</span>
							</ins>
							-
							<ins>
								...
								<span class="woocommerce-Price-currencySymbol"><?php echo $query_chk_list['currency_symbol']; ?></span>
								<?php
								if($min_vari_sale_price == $max_vari_sale_price) {

									echo number_format($max_vari_price * $query_chk_list['currency_value'], $decimails);

								} else {
									echo number_format($max_vari_sale_price * $query_chk_list['currency_value'], $decimails);
								}
								 ?>
								</span>
							</ins>
							<?php } else { ?>
								...
								<del>
									<span class="woocommerce-Price-currencySymbol"><?php echo $query_chk_list['currency_symbol']; ?></span>
									<?php echo number_format($min_vari_reg_price * $query_chk_list['currency_value'], $decimails); ?>
								</del>
								-
								<del>
									<span class="woocommerce-Price-currencySymbol"><?php echo $query_chk_list['currency_symbol']; ?></span>
								...
								<ins>
									<span class="woocommerce-Price-amount amount">
									<span class="woocommerce-Price-currencySymbol"><?php echo $query_chk_list['currency_symbol']; ?></span>
									<?php echo number_format($max_vari_sale_price * $query_chk_list['currency_value'], $decimails); ?>
									</span>
								</ins>
		...
		function filter_woocommerce_variation_price_html( $value, $object = null, $variation = null ) { 
			...
			if($query_chk_list!='') {
				...
				if($variation->sale_price!='') {
					...
					$value['price_html'] = '<span class="price"><del>' . $query_chk_list['currency_symbol'].number_format($variation->regular_price * $query_chk_list['currency_value'], $decimails) . '</del><ins>' . $query_chk_list['currency_symbol'].number_format($variation->sale_price * $query_chk_list['currency_value'], $decimails) . '</ins></span>';

				 } else { 

				$value['price_html'] = '<span class="price">' . $query_chk_list['currency_symbol'].number_format($variation->price * $query_chk_list['currency_value'], $decimails) . '</span>';
